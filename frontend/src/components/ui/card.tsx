...
function Card({
  ...
  size?: "default" | "sm"
  /** 카드 자체가 클릭 가능한 링크/버튼 역할일 때만 hover 모션을 켠다 (정적 카드는 그대로 유지) */
  clickable?: boolean
}) {
  return (
    <div
      ...
      className={cn(
        ...
        clickable &&
          "cursor-pointer transition-[transform,border-color] duration-(--dur-fast) ease-(--ease-out) hover:-translate-y-[3px] hover:border-primary/20",
        className
      )}
      {...props}
    />
